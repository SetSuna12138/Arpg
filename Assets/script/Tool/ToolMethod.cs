...
public class ToolMethod {
	...
	{
		//主角相对于目标的向量
		Vector3 srcLocalVect = t1 - t2.position;
		srcLocalVect.y = 0;
		//获取主角正前方的一个点
		Vector3 forwardLocalPos = t2.forward * 1 + t2.position;
		//获取正方向向量
		Vector3 forwardLocalVect = forwardLocalPos - t2.position;
		forwardLocalVect.y = 0;
		//计算角度
		return  Vector3.Angle(srcLocalVect, forwardLocalVect);
	}
